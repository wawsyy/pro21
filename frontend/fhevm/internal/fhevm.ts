...
async function getFHEVMRelayerMetadata(rpcUrl: string) {
  ...
  try {
    ...
    // Don't throw error, just return undefined to allow fallback to mock mode
    // This allows the app to work with regular Hardhat nodes using mock mode
    return undefined;
  } finally {
    rpc.destroy();
  }
...
export const createFhevmInstance = async (parameters: {
  ...
  if (isMock) {
    // For local Hardhat nodes, always use mock mode
    // Try to fetch FHEVM relayer metadata, but if not available, use default mock addresses
    ...
    // Use FHEVM relayer metadata if available, otherwise use default mock addresses
