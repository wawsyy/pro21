...
function useRainbowEthersSignerInternal(parameters: {
  initialMockChains?: Readonly<Record<number, string>>;
}): UseRainbowEthersSignerState {
  ...
  useEffect(() => {
    ...
    console.warn(
      `[useRainbowEthersSignerInternal] create new ethers.BrowserProvider(), chainId=${chainId}`
    );
    ...
    if (rpcUrl) {
      ...
      console.warn(
        `[useRainbowEthersSignerInternal] create new readonly provider ethers.JsonRpcProvider(${rpcUrl}), chainId=${chainId}`
      );
    } else {
      console.warn(
        `[useRainbowEthersSignerInternal] use ethers.BrowserProvider() as readonly provider, chainId=${chainId}`
      );
    }
...
const RainbowEthersSignerContext =
  createContext<UseRainbowEthersSignerState | undefined>(undefined);
...
export const RainbowEthersSignerProvider: React.FC<
  RainbowEthersSignerProviderProps
...
export function useRainbowEthersSigner() {
  ...
  if (context === undefined) {
    throw new Error(
      "useRainbowEthersSigner must be used within a RainbowEthersSignerProvider"
    );
  }
  return context;
}
